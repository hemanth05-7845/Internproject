...
SPRING_BASE_URL = (_ENGINE_HOST[:-4] if _ENGINE_HOST.endswith("/api") else _ENGINE_HOST) + "/api"


async def _get(path: str) -> dict | list:
    async with httpx.AsyncClient() as client:
        r = await client.get(f"{SPRING_BASE_URL}{path}", timeout=5.0)
        r.raise_for_status()
        return r.json()
...
async def _post(path: str, body: dict | None = None) -> dict:
    async with httpx.AsyncClient() as client:
        r = await client.post(f"{SPRING_BASE_URL}{path}", json=body or {}, timeout=5.0)
        r.raise_for_status()
        return r.json()
...
async def create_room(room_name: str, host_username: str) -> dict:
    return await _post("/rooms/create", {"roomName": room_name, "hostUsername": host_username})


async def join_room_by_code(room_code: str, username: str) -> dict:
    return await _post("/rooms/join-by-code", {"roomCode": room_code, "username": username})
...
async def submit_night_kill(room_id: str, target_player: str) -> dict:
    return await _post(f"/game/{room_id}/submit-night-kill", {"targetPlayer": target_player})


async def submit_police_guess(room_id: str, suspect_player: str) -> dict:
    return await _post(f"/game/{room_id}/submit-police-guess", {"suspectPlayer": suspect_player})


async def submit_doctor_save(room_id: str, saved_player: str) -> dict:
    return await _post(f"/game/{room_id}/submit-doctor-save", {"savedPlayer": saved_player})


# ---- Voting ----

async def submit_vote(room_id: str, voter_id: str, target_player: str) -> dict:
    return await _post(f"/rooms/{room_id}/vote",
                       {"voterId": voter_id, "targetPlayerId": target_player})


async def send_message(room_id: str, sender_username: str, content: str) -> dict:
    return await _post(f"/rooms/{room_id}/message",
